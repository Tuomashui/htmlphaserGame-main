...
window.onload = function() {
    let config = {
        ...
        scene: [BootGameScene, PlayGameScene, GameOverScene, FinalLevel],
        scale: {
...
export class FinalLevel extends Phaser.Scene {
    ...
    create() {
            ...
            this.cameras.main.setBackgroundColor('#000000');
                let text = this.add.text(500, 200, 'REACH 2 POINTS TO WIN', {
                fontFamily: 'Arial',
                fontSize: '44px',
                color: `rgb(${h}, ${j}, ${k})`
            }).setOrigin(0.5);
        ...
        let b = 5;
            ...
            for(let i = 0; i < 7; i++){
                let brick = this.brickGroup.create(Phaser.Math.Between(0, 1000), Phaser.Math.Between(0, 1000)
                    , "brick")
            ...
            this.cursors = this.input.keyboard.createCursorKeys();
              ...
              for(let i = 0; i < 3; i++){
                  let potion = this.potionGroup.create(Phaser.Math.Between(0, 1000), Phaser.Math.Between(0, 1000), "speedpotion");
            ...
            for(let i = 0; i < 10; i++){
                let pinecone = this.pineconeGroup.create(Phaser.Math.Between(0, 1000), Phaser.Math.Between(0, 1000), "pinecone");
